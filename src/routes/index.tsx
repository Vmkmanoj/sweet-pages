...
function MobileBook() {
  ...
  const [anim, setAnim] = useState<{ dir: "next" | "previous"; p: number; drag: boolean } | null>(null);
  const shell = useRef<HTMLElement>(null);
  const start = useRef<{ x: number; y: number; t: number } | null>(null);
  const busy = useRef(false);
  const last = pages.length - 1;

  const finish = useCallback((dir: "next" | "previous", complete: boolean) => {
    busy.current = true;
    setAnim({ dir, p: complete ? 1 : 0, drag: false });
    window.setTimeout(() => {
      if (complete) setIndex((i) => (dir === "next" ? Math.min(i + 1, last) : Math.max(i - 1, 0)));
      setAnim(null);
      busy.current = false;
    }, TURN_MS);
  }, [last]);

  const turn = useCallback((dir: "next" | "previous") => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === "ArrowRight") turn("next"); if (e.key === "ArrowLeft") turn("previous"); };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [turn]);

  const onDown = (e: React.PointerEvent) => { if (!busy.current) start.current = { x: e.clientX, y: e.clientY, t: performance.now() }; };
  const onMove = (e: React.PointerEvent) => {
    const s = start.current; if (!s || busy.current) return;
  ...
  const transition = anim && !anim.drag ? `transform ${TURN_MS}ms cubic-bezier(.33,.7,.25,1), opacity ${TURN_MS}ms ease` : "none";

  return (
    <>
      <section ref={shell} className="m-book" onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerCancel={onUp} aria-live="polite">
        <article className={`m-page ${getPage(base).kind === "cover" ? "cover-sheet" : ""}`} onClick={() => { if (!anim && base === 0) turn("next"); }}>
          <MenuPageContent page={getPage(base)} pageNumber={base + 1} onOpen={base === 0 ? () => turn("next") : undefined} />
          <div className="m-under-shade" style={{ opacity: anim ? 0.45 * (1 - Math.abs(angle) / 180) : 0, transition }} />
        </article>
        {sheet !== null && (
          <div className="m-sheet" style={{ transform: `rotateY(${angle}deg)`, transition }} aria-hidden="true">
            <div className={`m-face m-front ${getPage(sheet).kind === "cover" ? "cover-sheet" : ""}`}>
              <MenuPageContent page={getPage(sheet)} pageNumber={sheet + 1} onOpen={undefined} />
              <div className="m-curl" style={{ opacity: lift, transition }} />
            </div>
            <div className="m-face m-back"><div className="m-curl m-curl-back" style={{ opacity: lift, transition }} /></div>
            <div className="m-cast" style={{ opacity: lift * 0.9, transition }} />
          </div>
        )}
      </section>
      <nav className="menu-controls" aria-label="Menu page controls">
        <ControlButton label="Previous page" disabled={index === 0 || !!anim} onClick={() => turn("previous")}><ArrowLeft /></ControlButton>
        <div className="page-status"><span>{index + 1} / {pages.length}</span><small>← Swipe to turn →</small></div>
        <ControlButton label="Next page" disabled={index === last || !!anim} onClick={() => turn("next")}><ArrowRight /></ControlButton>
      </nav>
    </>
  );
}
